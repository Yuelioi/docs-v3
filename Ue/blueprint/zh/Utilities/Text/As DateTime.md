...
| Type | Name | Description |
| --- | --- | --- |
| text | Return Value | Converts a passed in date & time to a text, formatted as a date & time using an invariant timezone. This will use the given date & time as-is, so it's assumed to already be in the correct timezone. |
