---
title: Is Sense Enabled
order: 11
---
## Navigation

[Unreal Engine Blueprint API Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI) > [AI](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/AI) > [Perception](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/AI/Perception)

Returns if a sense is active. Note that this works only if given sense has been
already configured for this component instance

Target is AIPerception Component

## Inputs

| Type | Name | Description |
| --- | --- | --- |
| object | Target |  |
| class | Sense Class |  |

## Outputs

| Type | Name | Description |
| --- | --- | --- |
| boolean | Return Value | Returns if a sense is active. Note that this works only if given sense has beenalready configured for this component instance |
