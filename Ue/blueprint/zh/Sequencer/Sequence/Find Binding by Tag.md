---
display_name: Find Binding by Tag
order: 15
---
## Navigation

[Unreal Engine Blueprint API Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI) > [Sequencer](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Sequencer) > [Sequence](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Sequencer/Sequence)

Find the first object binding ID associated with the specified tag name (set up through RMB->Expose on Object bindings from within sequencer)

Target is Movie Scene Sequence

## Inputs

| Type | Name | Description |
| --- | --- | --- |
| object | Target |  |
| name | In Binding Name |  |

## Outputs

| Type | Name | Description |
| --- | --- | --- |
| struct | Return Value | Find the first object binding ID associated with the specified tag name (set up through RMB->Expose on Object bindings from within sequencer) |
