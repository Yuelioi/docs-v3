---
display_name: Move Binding Contents
order: 70
---
## Navigation

[Unreal Engine Blueprint API Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI) > [Sequencer](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Sequencer) > [Sequence](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Sequencer/Sequence)

Move all the contents (tracks, child bindings) of the specified binding ID onto another

Target is Movie Scene Binding Extensions

## Inputs

| Type | Name | Description |
| --- | --- | --- |
| exec | In |  |
| struct | Source Binding Id | The identifier of the binding ID to move all tracks and children from |
| struct | Destination Binding Id | The identifier of the binding ID to move the contents to |

## Outputs

| Type | Name | Description |
| --- | --- | --- |
| exec | Out |  |
