---
display_name: Find Scale Closest to World Location
order: 19
---
## Navigation

[Unreal Engine Blueprint API Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI) > [Spline](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Spline)

Given a location, in world space, return the spline's scale closest to the location.

Target is Spline Component

## Inputs

| Type | Name | Description |
| --- | --- | --- |
| object | Target |  |
| vector | World Location |  |

## Outputs

| Type | Name | Description |
| --- | --- | --- |
| vector | Return Value | Given a location, in world space, return the spline's scale closest to the location. |
