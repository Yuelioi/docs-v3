---
display_name: Text to Upper
order: 36
---
## Navigation

[Unreal Engine Blueprint API Reference](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI) > [Utilities](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Utilities) > [Text](https://dev.epicgames.com/documentation/en-us/unreal-engine/BlueprintAPI/Utilities/Text)

Transforms the text to uppercase in a culture correct way.
Note: The returned instance is linked to the original and will be rebuilt if the active culture is changed.

Target is Kismet Text Library

## Inputs

| Type | Name | Description |
| --- | --- | --- |
| text | In Text |  |

## Outputs

| Type | Name | Description |
| --- | --- | --- |
| text | Return Value | Transforms the text to uppercase in a culture correct way.@note The returned instance is linked to the original and will be rebuilt if the active culture is changed. |
